    ...
    con.connect(function (err) {
        bcrypt.genSalt(saltRounds, function (err, salt) {
            bcrypt.hash(req.body.pwd, salt, function (err, hash) {
                var sql = "CALL ADDUSER('" + req.body.uid + "','" + req.body.uname + "','" + req.body.emailid + "','" + hash + "')";
                con.query(sql, function (err, result) {
                    if (err) {
                        if (err.errno == 1062) {
...
app.post('/auth', function (request, response) {
    ...
    con.query('SELECT * FROM USER_DETAILS WHERE USER_ID = ? ', [username], function (error, results, fields) {
        if (results.length !== 0) {
        ...
        } else response.sendFile(path.join(__dirname, 'Login_error.html'));
    });
});
...
app.get('/existing', function (request, response) {
    if (request.session.loggedin) {
        var quer =
            'select PRESENTVIEW.BOOKING_ID,PRESENTVIEW.EXPRESS_NO,PRESENTVIEW.TRAIN_NO,'+
            'PRESENTVIEW.DEPARTURE_DATE as DEPARTURE_DATE,PRESENTVIEW.DEPARTURE_TIME,'+
            'PRESENTVIEW.TOTAL_FC_SEATS,PRESENTVIEW.TOTAL_SC_SEATS,PRESENTVIEW.TOTAL_COST '+
            'from EXPRESS,PRESENTVIEW where EXPRESS.EXPRESS_NO=PRESENTVIEW.EXPRESS_NO and ' +
            'PRESENTVIEW.USER_ID = ' +
            "'" +
            request.session.username +
            "'" +
...
app.post('/showtable', function (request, response) {
    if (request.session.loggedin) {
        ...
        var sqlquery =
            ...
            "EXPRESSVIEW.TOO, DATE_FORMAT(EXPRESSVIEW.DEPARTURE_DATE,'%d-%b-%Y') as DEPARTURE_DATE,"+
            " DEPARTURE_TIME from EXPRESSVIEW,TRAIN where " +
...
app.post('/tocancel', function (request, response) {
    if (request.session.loggedin) {
        var password = request.body.pwd;
        con.query('SELECT * FROM USER_DETAILS WHERE USER_ID = ? ', [request.session.username], function (error, results, fields) {
            if (results.length !== 0) {
                bcrypt.compare(password, results[0]['PASSWORD'], (error, result1) => {
                    if (result1 === true) {
            ...
            } else response.sendFile(path.join(__dirname, 'cancelerror.html'));
        });
    } else {
        response.redirect('/login');
    }
    ...
    if (request.session.loggedin) {
        ...
        con.query('SELECT * FROM USER_DETAILS WHERE USER_ID = ? ', [request.session.username], function (error, results, fields) {
            if (results.length !== 0) {
                bcrypt.compare(password, results[0]['PASSWORD'], (error, result1) => {
                    if (result1 === true) {
                        var sql ='insert into BOOKING(EXPRESS_NO,USER_ID,TRAIN_NO,TOTAL_FC_SEATS,TOTAL_SC_SEATS,TOTAL_COST) values(' +
                            "'" + final[0] + "'" + ',' + "'" + request.session.username + "'" + ',' +
                            "'" + final[1] + "'" + ',' + final[4] + ',' + final[5] + ',' + final[6] + ')';
                        con.query(sql, function (err, result) {
                            if (err) {
                                console.error('Error inserting!\n');
                                throw err;
                                ...
                                var updateview = 'CREATE OR REPLACE VIEW EXPRESSVIEW AS SELECT EXPRESS_NO, TRAIN_NO, FROMM,' +
            ...
            } else response.sendFile(path.join(__dirname, 'payerror.html'));
        });
    } else {
        response.redirect('/login');
    }
